<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jianzhishipinName" type="text" placeholder="视频名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in jianzhishipinTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.jianzhishipinPhoto?product.jianzhishipinPhoto.split(',')[0]:''"></image>
        <view class="listmr">
            <view class="col3 f30 elip mb15"
                  :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"rgba(255, 255, 255, 1)","textAlign":"center"}'>
                {{product.jianzhishipinName}}
            </view>
        </view>
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('jianzhishipin','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('jianzhishipin','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('jianzhishipin','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let jianzhishipinTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jianzhishipin_types'
		  	});
		  	this.jianzhishipinTypesList = jianzhishipinTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.jianzhishipinName = "";
			},
			//类别搜索
				...
				let params = {
                    ...
                    ,jianzhishipinDelete: 1
							...
							if (this.categoryName != 0) {
                                params.jianzhishipinTypes = this.categoryName
                            }

                let res = await this.$api.list(`jianzhishipin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jianzhishipin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.jianzhishipinName) {
                    searchForm['jianzhishipinName'] = this.searchForm.jianzhishipinName
                }
				if (this.searchForm.jianzhishipinName) {
					searchForm['jianzhishipinName'] = this.searchForm.jianzhishipinName
				}

                let res = await this.$api.list(`jianzhishipin`, searchForm);
				// 如果是第一页数据置空
